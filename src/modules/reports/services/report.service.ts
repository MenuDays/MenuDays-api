...
async create(
  ...
  await this.findRestaurantById(
    BigInt(restauranteId),
  );

  // Validar motivo.

  await this.findReasonById(
    BigInt(motivoId),
  );
...
private async findReasonById(
  ...
  if (!reason) {
    throw new NotFoundException(
      'El motivo del reporte no existe.',
    );
  }
 ...
 */
private async validateDuplicateReport(
  userId: bigint,
  restaurantId: bigint,
) {
  ...
  if (existingReport) {
    throw new BadRequestException(
      'Ya has reportado este restaurante.',
    );
  }
